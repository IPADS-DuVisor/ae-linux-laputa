...
static inline void ixy_tx_batch_busy_wait(struct virtio_device *dev, uint16_t qid,
...
static inline void ixy_read_stats(struct virtio_device *dev,
        struct device_stats *stats) {
    dev->read_stats(dev, stats);
}

int pktgen(struct virtio_device* dev) {
