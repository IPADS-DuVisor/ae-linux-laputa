...
void print_stats(struct device_stats* stats) {
	printf("[%p] RX: %zu bytes %zu packets\n", stats->device ? stats->device->mmio_addr : "???", stats->rx_bytes, stats->rx_pkts);
	printf("[%p] TX: %zu bytes %zu packets\n", stats->device ? stats->device->mmio_addr : "???", stats->tx_bytes, stats->tx_pkts);
}
...
void print_stats_diff(struct device_stats* stats_new, struct device_stats* stats_old, uint64_t nanos) {
	printf("[%p] RX: %d Mbit/s %.2f Mpps %d %d %d %d\n", stats_new->device ? stats_new->device->mmio_addr : "???",
...
void stats_init(struct device_stats* stats, struct virtio_device* dev) {
